...
public class PMappingHomography : MonoBehaviour {
	...
	void Update () {
		if(isHomographyEnabled){
			for(int i=0;i<PMappingControllerRef.initialInputPoints.Length;++i){
				source[i] = MVP.MultiplyPoint(PMappingControllerRef.initialInputPoints[i]);
				destination[i] = MVP.MultiplyPoint(PMappingControllerRef.finalOutputPoints[i]);
			}
